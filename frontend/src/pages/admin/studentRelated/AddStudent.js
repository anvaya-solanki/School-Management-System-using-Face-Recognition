...
const AddStudent = ({ situation }) => {
    ...
    const [embeddings, setEmbeddings] = useState(null);  // Store embeddings
    const [openWebcam, setOpenWebcam] = useState(false);
    const webcamRef = useRef(null);
    ...
    const submitHandler = async (event) => {
        ...
        try {
            // axios.post("http://localhost:5000/StudentReg", formData, {
            //     headers: {
            ...
            const res = await dispatch(registerUser(formData, role));
            if (res.payload.success) {
                setEmbeddings(res.payload.embeddings);  // Store embeddings
                navigate(-1);
                console.log("IN IF")
            } else {
